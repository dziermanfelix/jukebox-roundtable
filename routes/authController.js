...
import { createSession, webTokenMatchesJukebox, deleteSessionFromId, getSessionFromId } from './sessionController.js';
...
export const logout = async (req, res) => {
  ...
  const session = await getSessionFromId(req.body.sessionId);
  if (session && session.role === Role.STARTER) {
    await deleteJukeboxFromName(req.body.name);
  } else {
    await removeFromQueueOrder(req.body.name, req.body.sessionId);
    await deleteSessionFromId(req.body.sessionId);
  }
